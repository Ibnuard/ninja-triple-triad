...
export default function SuperAdminPage() {
  ...
  const {
    ...
    fetchPacks,
    addPack,
    updatePack: updateShopPack,
    deletePack,
  } = useShopStore();
  const [editingPackId, setEditingPackId] = useState<string | null>(null);
  const [packFormData, setPackFormData] = useState<
    Omit<ShopPack, "id" | "created_at">
  >({
    name: "New Pack",
  ...
  return (
    <div className="min-h-screen bg-black text-white flex flex-col p-4 md:p-8 font-mono">
      <div className="max-w-[1400px] mx-auto w-full">
        {/* Header - Shop Style */}
        <header className="flex items-center justify-between border-b border-white/5 bg-black/40 backdrop-blur-md p-4 md:p-6 mb-8 rounded-t-2xl">
