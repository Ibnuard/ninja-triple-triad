...
interface ShopStore {
  ...
  addPack: (
    pack: Omit<ShopPack, "id" | "created_at">
  ) => Promise<{ success: boolean; error?: string }>;
  updatePack: (
    id: string,
    pack: Partial<ShopPack>
  ) => Promise<{ success: boolean; error?: string }>;
  ...
  buyPack: async (packId, userId) => {
    ...
    for (const rule of pack.config) {
      for (let i = 0; i < rule.count; i++) {
        ...
        } else if (rule.type === "filtered_pool") {
          // Start with all cards
          pool = allCards;

          // Filter by Rarities if provided
          if (rule.value.rarities && rule.value.rarities.length > 0) {
            pool = pool.filter((c) =>
              rule.value.rarities.includes(c.rarity || "common")
            );
          }

          // Filter by CP Range if provided
          if (rule.value.cpRange) {
            const [min, max] = rule.value.cpRange;
            pool = pool.filter((c) => (c.cp || 0) >= min && (c.cp || 0) <= max);
          }
        }
        ...
        if (pool.length > 0) {
          ...
          if (
            collectionIds.has(randomCard.id) ||
            generatedCards.some((c) => c.id === randomCard.id)
          ) {
    ...
    if (newCardsToInsert.length > 0) {
      const { error: insertError } = await supabase
        .from("user_cards")
        .insert(newCardsToInsert);
      if (insertError)
        return { success: false, error: "Failed to add cards to collection" };
    }
